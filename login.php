...
/**
 * REFACTOR: Centralized Redirection Helper
 */
function redirect_by_role($role, $p_id) {
    if ($role === 'patient' && !empty($p_id)) {
        header("Location: patients/view.php?id=" . $p_id);
    } else {
...
if (isset($_SESSION['user_id'])) {
    redirect_by_role($_SESSION['role'], $_SESSION['patient_id']);
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // 3. Unified Cryptographic Verification
    // Works for both Admin and Patients now
    if ($u && password_verify($pass, $u['password'])) {
        
        // 4. Session Hardening: Prevent hijacking
        ...
        redirect_by_role($u['role'], $u['patient_id']);
    } else {
