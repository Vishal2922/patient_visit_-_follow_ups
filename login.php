...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($u) {
        ...
        // 2. Hybrid Logic: Plain text for Admin, Hash for Patient
        if ($u['role'] === 'admin') {
            // Check Admin using plain text comparison
            if ($pass === $u['password']) {
                $isValid = true;
            }
        } else {
            // Check Patients using secure cryptographic verification
        ...
        if ($isValid) {
            ...
            // Role-based routing
            if ($u['role'] === 'patient' && !empty($u['patient_id'])) {
            ...
            $error = "Invalid medical portal credentials.";
        }
    } else {
        $error = "Invalid medical portal credentials.";
    }
